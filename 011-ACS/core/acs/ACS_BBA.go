...
/* BBA（异步二进制协议, Binary Byzantine Agreement)
异步二元共识是一个密码学原语，用于在异步环境下让所有节点对于 0 或 1 达成共识。
对应后面的MVBA异步多值共识
BBA 是多轮协议，每一轮都会经历：
BVAL （广播二进制值）阶段 -> AUX （广播辅助值）阶段 -> coin 阶段
...
v 加入 binaryValues
    |
    v
第一次有值进入 binaryValues 时，广播 AUX(v)
    |
    v
收到 N-F 个 AUX
    |
...
获取 common coin
    |
    v
如果 AUX 候选值唯一且等于 coin
        产生 decision/output
    否则
        下一轮 estimated = coin 或唯一 AUX 值
    |
    v
进入下一 round，广播新的 BVAL(estimated)
*/
...
func NewBBAWithCoin(cfg Config, commonCoin *CommonCoin) *BBA {
	bba := &BBA{
		Config:          cfg,
		recvBinaryValue: make(map[uint64]bool),
		recvAux:         make(map[uint64]bool),
...
func (b *BBA) outputAgreement() {
	...
	var coin bool
	if b.commonCoin != nil {
		coin = b.commonCoin.GetCoin(b.epoch)
	} else {
		// 如果没有公共硬币，使用简单的确定性方法
		coin = b.epoch%2 == 0
	}
